...
# ds_daily %>% glimpse()
ds_usts <- readr::read_csv(config$path_input_jh_usts) # timeseries, county level
# ds_usts %>% glimpse()
...
ggsave("./analysis/us-response-2/tests-by-state.jpg",g,device = "jpg",width = 18, height = 12, dpi = "retina" )
...
# ------ epi-function-0 ----------------------
# computation function that works on county-level
...
quick_save <- function(g,name,...){
  ggplot2::ggsave(
    ...
    plot     = g,
    device   = png,
    path     = "./analysis/us-response-2/prints/", # female marital educ poor_healt
    # width    = width,
    # height   = height,
    # units = "cm",
    # dpi      = 'retina',
...
# State level
d <- ds_jh_state %>%
  compute_epi(c("date", "province_state","division", "region", "country_region"), long = T)
g <-  d %>%
  ggplot(aes(x=date, y = value, group = province_state))+
  geom_line(alpha = .2)+
  ...
  facet_wrap(~metric, scales = "free", ncol = 4)
g %>% quick_save("state", width = 1600, height = 900, res = 120)
