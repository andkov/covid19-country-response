...
quick_save2 <- function(g,name,...){
  ggplot2::ggsave(
    ...
    plot     = g,
    device   = "jpg",
    path     = "./analysis/us-response-2/prints-6/",
    # width    = width,
    # height   = height,
    # units = "cm",
    dpi      = 'retina',
...
g <-  d %>%
  ...
  geom_text(aes(label = event_description_short,y = seq(65,50,-5), x = as.Date("2020-03-05")),size =3,data = d %>% filter(!is.na(event_n), state=="New York", metric == "Cases (7DA/100K)"),color ="black",hjust=0)+
  # legend for Florida
  geom_point(shape = 19, size = 3,data = d %>% filter(!is.na(event_n), state=="Florida", metric == "Cases (7DA/100K)"), aes(y = seq(95,75,-5), x = as.Date("2020-04-27")) )+
  geom_text(aes(label = event_n,y = seq(95,75,-5), x = as.Date("2020-04-27")),size =2,data = d %>% filter(!is.na(event_n), state=="Florida", metric == "Cases (7DA/100K)"),color ="white")+
  geom_text(aes(label = event_description_short,y = seq(95,75,-5), x = as.Date("2020-04-30")),size =3,data = d %>% filter(!is.na(event_n), state=="Florida", metric == "Cases (7DA/100K)"),color ="black",hjust=0)+
  ...
  # lemon::facet_rep_wrap(~metric,scales = "free_y", ncol = 1,repeat.tick.labels = TRUE)+
  scale_x_date(date_breaks = "1 month", date_labels = "%b" )+
  scale_y_continuous(breaks = seq(0,120,10),labels = scales::comma_format(),sec.axis = sec_axis(~ . * 1,breaks = seq(0,120,10) ))+
  scale_color_manual(values = state_colors)+
  labs(title = "COVID-19 Cases and Tests for 3 US States",
       x = "2020", y = NULL, color = NULL, linetype=NULL, caption = "Note: Testing data are unreliable after Oct 26")+
  # theme(legend.position = c(0.7, 0.55),legend.background = element_rect(colour=NA, fill=NA), legend.box.just = "left", legend.spacing.y =unit(-.2, 'cm'))
  theme(legend.position = c(0.1, 0.85),legend.background = element_rect(colour=NA, fill=NA), legend.box.just = "bottom", legend.spacing.y =unit(-.1, 'cm'))
...
table1 %>% gtsave("./analysis/us-response-2/prints-6/descrip-table.png")
